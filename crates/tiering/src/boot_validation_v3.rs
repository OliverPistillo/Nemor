//! Phase 6 validation-only boot contract v3.
//!
...
pub const BOOT_VALIDATION_CONTRACT_VERSION_V3: &str = "tiering-boot-validation-v3";
pub const PREPARED_MANIFEST_SCHEMA_V3: &str = "tiering-boot-prepared-manifest-v3";
pub const DURABLE_TRANSACTION_SCHEMA_V3: &str = "tiering-boot-transaction-v3";
pub const PREFLIGHT_SCHEMA_V3: &str = "tiering-boot-preflight-v3";
pub const APPLY_EVIDENCE_SCHEMA_V3: &str = "tiering-boot-apply-evidence-v3";
pub const POST_BOOT_EVIDENCE_SCHEMA_V3: &str = "tiering-post-boot-evidence-v3";
pub const FINAL_RESTORE_SCHEMA_V3: &str = "tiering-final-restore-evidence-v3";
pub const PROFILE_BENCHMARK_EVIDENCE_V3: &str = "tiering-profile-benchmark-v3";
pub const ZRAM_BASELINE_EVIDENCE_V2: &str = "tiering-zram-baseline-v2";
pub const ACTIVATION_EVIDENCE_SCHEMA_V3: &str = "tiering-activation-evidence-v3";
pub const RECOVERY_EVIDENCE_SCHEMA_V3: &str = "tiering-recovery-evidence-v3";
pub const WORKLOAD_PROTOCOL_V1: &str = "tiering-bounded-workload-v1";
...
pub enum BootValidationCommandV3 {
    Prepare,
    UserPreflight,
    RootPreflight,
    MeasureBaseline,
...
pub enum BootValidationV3Error {
    #[error("v3 schema or contract mismatch")]
    Version,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StagedBinaryPlanV3 {
    pub source: BinaryIdentityV3,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BlockLayerIdentityV3 {
    pub path: PathBuf,
    pub kind: String,
    pub major: u32,
    pub minor: u32,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PhysicalDeviceIdentityV3 {
    pub path: PathBuf,
    pub major: u32,
    pub minor: u32,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FilesystemIdentityV3 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StorageTopologyIdentityV3 {
    pub storage_profile_version: String,
    pub profile: StorageProfile,
    pub chain: Vec<BlockLayerIdentityV3>,
    pub physical: PhysicalDeviceIdentityV3,
    pub filesystem: FilesystemIdentityV3,
    pub composite: bool,
    pub ambiguous: bool,
    pub confidence: String,
}

impl StorageTopologyIdentityV3 {
    pub fn validate(&self) -> Result<(), BootValidationV3Error> {
        ...
        {
            return Err(BootValidationV3Error::Topology);
        }
        let stable = self
            .physical
            .serial
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ZswapIdentityV3 {
    pub parameters: BTreeMap<String, String>,
}

impl ZswapIdentityV3 {
    fn validate(&self) -> Result<(), BootValidationV3Error> {
        let required = [
            "enabled",
            "compressor",
            "zpool",
        ...
        if required.iter().any(|k| !self.parameters.contains_key(*k)) {
            return Err(BootValidationV3Error::Zswap);
        }
        Ok(())
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BootEntryIdentityV3 {
    pub id: String,
    pub path: PathBuf,
    pub sha256: String,
    pub title: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReferencedBootFileV3 {
    pub path: PathBuf,
    pub sha256: String,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct OwnedArtifactV3 {
    pub kind: OwnedArtifactKindV3,
    pub path: PathBuf,
    pub sha256: String,
    pub mode: u32,
    pub owner_uid: u32,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BootIdentityV3 {
    pub bootloader: String,
    pub bootloader_version: String,
    pub current_entry: BootEntryIdentityV3,
    pub default_entry: BootEntryIdentityV3,
    ...
    pub referenced_files: Vec<ReferencedBootFileV3>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PreparedManifestPayloadV3 {
    ...
    pub binaries: BTreeMap<String, BinaryIdentityV3>,
    pub config_path: PathBuf,
    pub config_sha256: String,
    pub material_environment_sha256: String,
    pub topology: StorageTopologyIdentityV3,
    pub baseline_swaps: Vec<SwapIdentityV3>,
    pub protected_zram: ZramIdentityV3,
    pub baseline_zswap: ZswapIdentityV3,
    pub experimental_zswap: BTreeMap<String, String>,
    pub boot: BootIdentityV3,
    pub experimental_entry: BootEntryIdentityV3,
    pub validation_marker: String,
    pub swapfile: SwapIdentityV3,
    pub owned_artifacts: Vec<OwnedArtifactV3>,
    pub transaction_root: PathBuf,
    pub workload: WorkloadContractV3,
    pub staged_helper: StagedBinaryPlanV3,
    pub recovery_entry: String,
    pub production_activation: bool,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct TieringBootValidationPreparedManifestV3 {
    pub schema: String,
    pub payload: PreparedManifestPayloadV3,
    pub payload_sha256: String,
}

impl TieringBootValidationPreparedManifestV3 {
    pub fn seal(payload: PreparedManifestPayloadV3) -> Self {
        let payload_sha256 = canonical_json_sha256(&payload);
        Self {
            schema: PREPARED_MANIFEST_SCHEMA_V3.to_owned(),
            payload,
            payload_sha256,
        }
    }
    ...
    pub fn validate(&self) -> Result<(), BootValidationV3Error> {
        let p = &self.payload;
        if self.schema != PREPARED_MANIFEST_SCHEMA_V3
            || p.contract_version != BOOT_VALIDATION_CONTRACT_VERSION_V3
            || p.rule_version != TIERING_RULE_VERSION
        {
            return Err(BootValidationV3Error::Version);
        }
        if canonical_json_sha256(p) != self.payload_sha256 {
            return Err(BootValidationV3Error::Payload);
        }
        if !valid_id(&p.validation_id) || !is_commit(&p.source_commit) {
            return Err(BootValidationV3Error::Identity("source"));
        }
        if p.validation_marker != format!("nemor.phase6_validation={}", p.validation_id)
            || !p.config_path.is_absolute()
        {
            return Err(BootValidationV3Error::Identity("bounded inputs"));
        }
        for hash in [
            ...
            if !is_sha256(hash) {
                return Err(BootValidationV3Error::Identity("hash"));
            }
        }
        ...
        {
            return Err(BootValidationV3Error::Identity("binary"));
        }
        let validator = p
            .binaries
            .get("nemor-tiering-boot-validation")
            .ok_or(BootValidationV3Error::Identity("validator binary"))?;
        ...
        {
            return Err(BootValidationV3Error::Identity("staged helper"));
        }
        p.topology.validate()?;
        ...
        {
            return Err(BootValidationV3Error::Identity("baseline swaps"));
        }
        ...
        {
            return Err(BootValidationV3Error::Identity("recovery"));
        }
        ...
        for artifact in &p.owned_artifacts {
            ...
            if !paths.insert(artifact.path.clone())
                ...
                || (artifact.kind != OwnedArtifactKindV3::HelperBinary
                    && canonical_bytes_sha256(&artifact.content) != artifact.sha256)
            {
                return Err(BootValidationV3Error::Identity("artifact"));
            }
        }
        if p.workload.protocol != WORKLOAD_PROTOCOL_V1
            || p.workload.bytes == 0
        ...
        {
            return Err(BootValidationV3Error::Identity("workload"));
        }
        Ok(())
    }
}
...
fn validate_boot(p: &PreparedManifestPayloadV3) -> Result<(), BootValidationV3Error> {
    let b = &p.boot;
    ...
    {
        return Err(BootValidationV3Error::Entry);
    }
    ...
    let entry_artifact = p
        .owned_artifacts
        .iter()
        .find(|a| a.kind == OwnedArtifactKindV3::Type1Entry)
        .ok_or(BootValidationV3Error::Entry)?;
    if entry_artifact.content != render_type1_entry_v3(&p.experimental_entry).into_bytes() {
        return Err(BootValidationV3Error::Entry);
    }
    ...
    let unit_artifact = p
        .owned_artifacts
        .iter()
        .find(|a| a.kind == OwnedArtifactKindV3::ValidationUnit)
        .ok_or(BootValidationV3Error::Entry)?;
    let helper = p
        .owned_artifacts
        .iter()
        .find(|a| a.kind == OwnedArtifactKindV3::HelperBinary)
        .ok_or(BootValidationV3Error::Identity("staged helper artifact"))?;
    if helper.path != p.staged_helper.destination
        || helper.sha256 != p.staged_helper.source.sha256
        || helper.mode != 0o755
        || !helper.content.is_empty()
    {
        return Err(BootValidationV3Error::Identity("staged helper artifact"));
    }
    if unit_artifact.content
        != render_validation_unit_v3(p, &p.staged_helper.destination).into_bytes()
    {
        return Err(BootValidationV3Error::Entry);
    }
    Ok(())
...
#[must_use]
pub fn render_type1_entry_v3(entry: &BootEntryIdentityV3) -> String {
...
#[must_use]
pub fn render_validation_unit_v3(p: &PreparedManifestPayloadV3, validator: &Path) -> String {
    format!(
...
fn validate_swap(p: &PreparedManifestPayloadV3) -> Result<(), BootValidationV3Error> {
    ...
    {
        return Err(BootValidationV3Error::Swap);
    }
    Ok(())
}

fn validate_owned_path(
    p: &PreparedManifestPayloadV3,
    a: &OwnedArtifactV3,
) -> Result<(), BootValidationV3Error> {
    ...
    {
        return Err(BootValidationV3Error::Path);
    }
    let allowed = match a.kind {
        OwnedArtifactKindV3::Type1Entry => a.path == p.experimental_entry.path,
        OwnedArtifactKindV3::ValidationUnit => {
            a.path == Path::new(UNIT_ROOT).join(format!("nemor-phase6-{}.service", p.validation_id))
        }
        OwnedArtifactKindV3::HelperBinary => {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DurableTransactionPayloadV3 {
    pub validation_id: String,
    pub manifest_sha256: String,
    pub stage: TransactionStageV3,
    ...
    pub applied_swap_identity: Option<SwapIdentityV3>,
    pub evidence_hashes: BTreeMap<String, String>,
    pub mutation_records: Vec<MutationRecordV3>,
    pub primary_error: Option<String>,
    pub failed_from_stage: Option<TransactionStageV3>,
    ...
    pub post_boot_summary: Option<PostBootSummaryV3>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PostBootSummaryV3 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DurableTransactionV3 {
    pub schema: String,
    pub payload: DurableTransactionPayloadV3,
    pub payload_sha256: String,
}

impl DurableTransactionV3 {
    pub fn new(
        manifest: &TieringBootValidationPreparedManifestV3,
        boot_id: String,
    ) -> Result<Self, BootValidationV3Error> {
        manifest.validate()?;
        let p = &manifest.payload;
        let payload = DurableTransactionPayloadV3 {
            validation_id: p.validation_id.clone(),
            manifest_sha256: canonical_json_sha256(manifest),
            stage: TransactionStageV3::Prepared,
            ...
            unit_registry: p
                .owned_artifacts
                .iter()
                .filter(|a| a.kind == OwnedArtifactKindV3::ValidationUnit)
                .map(|a| a.path.clone())
                .collect(),
    ...
    pub fn seal(payload: DurableTransactionPayloadV3) -> Self {
        let payload_sha256 = canonical_json_sha256(&payload);
        Self {
            schema: DURABLE_TRANSACTION_SCHEMA_V3.to_owned(),
            payload,
            payload_sha256,
        }
    }
    pub fn validate(&self) -> Result<(), BootValidationV3Error> {
        if self.schema != DURABLE_TRANSACTION_SCHEMA_V3
            || canonical_json_sha256(&self.payload) != self.payload_sha256
        {
            Err(BootValidationV3Error::Payload)
        } else {
            Ok(())
        }
    }
    pub fn transition(&mut self, next: TransactionStageV3) -> Result<(), BootValidationV3Error> {
        self.validate()?;
        if !legal_transition_v3(self.payload.stage, next) {
            return Err(BootValidationV3Error::Transition);
        }
    ...
    pub fn record_intent(&mut self, operation: &str, target: PathBuf) {
        self.payload.mutation_records.push(MutationRecordV3 {
    ...
    pub fn complete_last(&mut self, evidence_sha256: String) -> Result<(), BootValidationV3Error> {
        let last = self
            .payload
            .mutation_records
            .last_mut()
            .ok_or(BootValidationV3Error::Transition)?;
        last.completed = true;
...
pub fn legal_transition_v3(from: TransactionStageV3, to: TransactionStageV3) -> bool {
    use TransactionStageV3::*;
    matches!(
        (from, to),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PreflightObservationV3 {
    pub schema: String,
    pub uid: u32,
    pub gid: u32,
...
#[must_use]
pub fn derived_preflight_ready_v3(
    manifest: &TieringBootValidationPreparedManifestV3,
    o: &PreflightObservationV3,
) -> bool {
    [
        o.source_matches,
        o.binaries_match,
...
pub fn validate_preflight_v3(
    manifest: &TieringBootValidationPreparedManifestV3,
    o: &PreflightObservationV3,
    root: bool,
) -> Result<(), BootValidationV3Error> {
    manifest.validate()?;
    if o.schema != PREFLIGHT_SCHEMA_V3 || o.mutation_count != 0 {
    ...
    if o.host_identity_sha256 != manifest.payload.material_environment_sha256 {
        return Err(BootValidationV3Error::Identity("material environment"));
    }
    if root
        && (o.uid != 0
    ...
    {
        return Err(BootValidationV3Error::SudoIdentity);
    }
    if !root && (o.uid != manifest.payload.prepared_uid || o.gid != manifest.payload.prepared_gid) {
        return Err(BootValidationV3Error::Identity("preparing user"));
    }
    let derived_ready = derived_preflight_ready_v3(manifest, o);
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ActualPostBootObservationV3 {
    pub schema: String,
    pub boot_id: String,
    ...
    pub runtime_observation: RuntimeObserveOnlyEvidenceV3,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RuntimeObserveOnlyEvidenceV3 {
    pub config_sha256: String,
    pub configured_observe_only: bool,
    pub nemord_active: bool,
    pub nemord_binary: Option<RuntimeBinaryIdentityV3>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RuntimeBinaryIdentityV3 {
    pub path: PathBuf,
    pub sha256: String,
    pub pid: u32,
    pub start_ticks: u64,
...
pub fn validate_actual_post_boot_v3(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &DurableTransactionV3,
    o: &ActualPostBootObservationV3,
) -> Result<(), BootValidationV3Error> {
    manifest.validate()?;
    tx.validate()?;
    if o.schema != POST_BOOT_EVIDENCE_SCHEMA_V3
        || tx.payload.stage != TransactionStageV3::PostBootMeasuring
        || o.boot_id == tx.payload.baseline_boot_id
    {
        return Err(BootValidationV3Error::Measurement("boot identity"));
    }
    let p = &manifest.payload;
    ...
    {
        return Err(BootValidationV3Error::Measurement("boot readback"));
    }
    if o.zswap
        != (ZswapIdentityV3 {
    ...
    {
        return Err(BootValidationV3Error::Measurement("runtime identity"));
    }
    let swap = o
        .swaps
        .iter()
        .find(|s| s.path == p.swapfile.path)
        .ok_or(BootValidationV3Error::Measurement("swap absent"))?;
    if !swap.active
    ...
    {
        return Err(BootValidationV3Error::Measurement(
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BaselineRestoreObservationV3 {
    pub schema: String,
    pub boot_id: String,
    pub booted_entry: String,
    pub command_line: String,
...
pub fn verify_baseline_before_cleanup_v3(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &DurableTransactionV3,
    o: &BaselineRestoreObservationV3,
) -> Result<(), BootValidationV3Error> {
    let p = &manifest.payload;
    if o.schema != FINAL_RESTORE_SCHEMA_V3
        || tx.payload.stage != TransactionStageV3::BaselineBoot
        || o.boot_id == tx.payload.baseline_boot_id
        ...
        || o.production_activation
    {
        return Err(BootValidationV3Error::Readback(
...
pub struct SameHostZramBaselineEvidenceV2 {
    pub schema: String,
    pub validation_id: String,
    pub source_commit: String,
    pub source_state_sha256: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BaselineMeasurementObservationV3 {
    pub schema: String,
    pub validation_id: String,
    pub boot_id: String,
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct SameHostProfileEvidenceV3 {
    pub schema: String,
    pub validation_id: String,
    pub profile: StorageProfile,
    pub source_commit: String,
...
pub fn matching_same_host_evidence_v3(
    b: &SameHostZramBaselineEvidenceV2,
    p: &SameHostProfileEvidenceV3,
    profile: StorageProfile,
) -> bool {
    b.schema == ZRAM_BASELINE_EVIDENCE_V2
        && p.schema == PROFILE_BENCHMARK_EVIDENCE_V3
        && p.profile == profile
        ...
        && b.workload_protocol == WORKLOAD_PROTOCOL_V1
        && p.workload_protocol == WORKLOAD_PROTOCOL_V1
        && p.backing_write_confidence == "bounded-physical-device-attributed"
}

pub fn recovery_action_v3(stage: TransactionStageV3, currently_experimental: bool) -> &'static str {
    use TransactionStageV3::*;
    match stage {
...
pub trait BootLifecycleBackendV3 {
    fn persist_transaction(&mut self, transaction: &DurableTransactionV3) -> Result<(), String>;
    ...
    fn create_swapfile(
        &mut self,
        manifest: &TieringBootValidationPreparedManifestV3,
    ) -> Result<SwapIdentityV3, String>;
    fn create_artifact(&mut self, artifact: &OwnedArtifactV3) -> Result<(), String>;
    fn stage_helper(&mut self, plan: &StagedBinaryPlanV3) -> Result<(), String>;
    fn staged_helper_matches(&self, plan: &StagedBinaryPlanV3) -> bool;
    fn artifact_matches(&self, artifact: &OwnedArtifactV3) -> bool;
    fn artifact_absent(&self, artifact: &OwnedArtifactV3) -> bool;
    fn sync_parents(
        &mut self,
        manifest: &TieringBootValidationPreparedManifestV3,
    ) -> Result<(), String>;
    fn remove_artifact(&mut self, artifact: &OwnedArtifactV3) -> Result<(), String>;
    fn remove_swapfile(
        &mut self,
        manifest: &TieringBootValidationPreparedManifestV3,
    ) -> Result<(), String>;
    fn swapfile_absent(&self, manifest: &TieringBootValidationPreparedManifestV3) -> bool;
    ...
    fn collect_zram_baseline(
        &mut self,
        manifest: &TieringBootValidationPreparedManifestV3,
    ) -> Result<BaselineMeasurementObservationV3, String>;
    fn collect_post_boot(
        &mut self,
        manifest: &TieringBootValidationPreparedManifestV3,
    ) -> Result<ActualPostBootObservationV3, String>;
    fn collect_baseline(
        &self,
        manifest: &TieringBootValidationPreparedManifestV3,
    ) -> Result<BaselineRestoreObservationV3, String>;
    fn seal_archive(&mut self, transaction: &DurableTransactionV3) -> Result<(), String>;
}

fn persist_or_fail<B: BootLifecycleBackendV3>(
    backend: &mut B,
    tx: &DurableTransactionV3,
) -> Result<(), BootValidationV3Error> {
    backend
        .persist_transaction(tx)
        .map_err(|_| BootValidationV3Error::Readback("durable transaction persistence"))
}

fn fail_transaction<B: BootLifecycleBackendV3>(
    backend: &mut B,
    tx: &mut DurableTransactionV3,
    primary: String,
    secondary: Vec<String>,
) -> BootValidationV3Error {
    ...
    tx.payload.stage = TransactionStageV3::Failed;
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    let _ = backend.persist_transaction(tx);
    BootValidationV3Error::Readback("bounded mutation failed; transaction retained")
}

fn rollback_partial_apply<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    created: &[&OwnedArtifactV3],
    backend: &mut B,
) -> Vec<String> {
...
pub fn initialize_and_measure_baseline_v3<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    root_preflight: &PreflightObservationV3,
    boot_id: String,
    backend: &mut B,
) -> Result<DurableTransactionV3, BootValidationV3Error> {
    validate_preflight_v3(manifest, root_preflight, true)?;
    let mut tx = DurableTransactionV3::new(manifest, boot_id)?;
    backend
        .create_transaction_root(manifest)
        .map_err(|_| BootValidationV3Error::Readback("create transaction root"))?;
    persist_or_fail(backend, &tx)?;
    backend
        .copy_prepared_manifest(manifest)
        .map_err(|_| BootValidationV3Error::Readback("copy prepared manifest"))?;
    let preflight_bytes =
        serde_json::to_vec(root_preflight).map_err(|_| BootValidationV3Error::Payload)?;
    backend
        .persist_evidence("root-preflight-v3.json", &preflight_bytes)
        .map_err(|_| BootValidationV3Error::Readback("persist root preflight"))?;
    tx.payload.evidence_hashes.insert(
        "root-preflight-v3.json".to_owned(),
        canonical_bytes_sha256(&preflight_bytes),
    );
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.transition(TransactionStageV3::RootPreflighted)?;
    persist_or_fail(backend, &tx)?;
    tx.record_intent(
    ...
    persist_or_fail(backend, &tx)?;
    tx.transition(TransactionStageV3::BaselineMeasuring)?;
    persist_or_fail(backend, &tx)?;
    let observation = backend
        .collect_zram_baseline(manifest)
        .map_err(|primary| fail_transaction(backend, &mut tx, primary, Vec::new()))?;
    if observation.schema != ZRAM_BASELINE_EVIDENCE_V2
        ...
        || observation.workload_protocol != WORKLOAD_PROTOCOL_V1
    ...
    let bytes = serde_json::to_vec(&observation).map_err(|_| BootValidationV3Error::Payload)?;
    backend
        .persist_evidence("zram-baseline-evidence-v2.json", &bytes)
        .map_err(|_| BootValidationV3Error::Readback("persist zram baseline"))?;
    tx.payload.evidence_hashes.insert(
        "zram-baseline-evidence-v2.json".into(),
        canonical_bytes_sha256(&bytes),
    );
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.transition(TransactionStageV3::BaselineMeasured)?;
    persist_or_fail(backend, &tx)?;
    Ok(tx)
}
...
pub fn apply_exact_transaction_v3<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &mut DurableTransactionV3,
    backend: &mut B,
) -> Result<(), BootValidationV3Error> {
    manifest.validate()?;
    tx.validate()?;
    if tx.payload.stage != TransactionStageV3::BaselineMeasured
        || tx.payload.manifest_sha256 != canonical_json_sha256(manifest)
        || !tx
            .payload
            .evidence_hashes
    ...
    tx.transition(TransactionStageV3::Applying)?;
    persist_or_fail(backend, tx)?;
    let mut completed_artifacts: Vec<&OwnedArtifactV3> = manifest
        .payload
        .owned_artifacts
        .iter()
        .filter(|artifact| artifact.kind == OwnedArtifactKindV3::HelperBinary)
        .collect();
    tx.record_intent(
    ...
    for artifact in &manifest.payload.owned_artifacts {
        if artifact.kind == OwnedArtifactKindV3::HelperBinary {
            if !backend.staged_helper_matches(&manifest.payload.staged_helper) {
                return Err(fail_transaction(
                    backend,
                    tx,
        ...
        persist_or_fail(backend, tx)?;
        let created = if artifact.kind == OwnedArtifactKindV3::HelperBinary {
        ...
        let matches = if artifact.kind == OwnedArtifactKindV3::HelperBinary {
    ...
    let apply_evidence = serde_json::to_vec(&serde_json::json!({
        "schema": APPLY_EVIDENCE_SCHEMA_V3,
    ...
    .map_err(|_| BootValidationV3Error::Payload)?;
    backend
        .persist_evidence("apply-evidence-v3.json", &apply_evidence)
        .map_err(|_| BootValidationV3Error::Readback("persist apply evidence"))?;
    tx.payload.evidence_hashes.insert(
        "apply-evidence-v3.json".into(),
        canonical_bytes_sha256(&apply_evidence),
    );
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.transition(TransactionStageV3::Applied)?;
    persist_or_fail(backend, tx)?;
    Ok(())
}

pub fn select_exact_one_shot_v3<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &mut DurableTransactionV3,
    backend: &mut B,
) -> Result<(), BootValidationV3Error> {
    ...
    tx.validate()?;
    if !backend.staged_helper_matches(&manifest.payload.staged_helper) {
        return Err(BootValidationV3Error::Readback("staged helper"));
    }
    tx.transition(TransactionStageV3::OneShotSelecting)?;
    persist_or_fail(backend, tx)?;
    tx.record_intent(
        "set_oneshot",
    ...
    let one_shot_bytes = serde_json::to_vec(&serde_json::json!({
        "schema":"tiering-one-shot-evidence-v3",
    ...
    .map_err(|_| BootValidationV3Error::Payload)?;
    backend
        .persist_evidence("one-shot-evidence-v3.json", &one_shot_bytes)
        .map_err(|_| BootValidationV3Error::Readback("persist one-shot evidence"))?;
    tx.payload.evidence_hashes.insert(
        "one-shot-evidence-v3.json".into(),
        canonical_bytes_sha256(&one_shot_bytes),
    );
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.complete_last(canonical_json_sha256(&manifest.payload.experimental_entry))?;
    tx.transition(TransactionStageV3::OneShotSelected)?;
    persist_or_fail(backend, tx)
}
...
pub fn collect_and_validate_post_boot_v3<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &mut DurableTransactionV3,
    backend: &mut B,
) -> Result<ActualPostBootObservationV3, BootValidationV3Error> {
    if tx.payload.stage != TransactionStageV3::ActivationVerified {
        return Err(BootValidationV3Error::Transition);
    }
    if !backend.staged_helper_matches(&manifest.payload.staged_helper) {
        return Err(BootValidationV3Error::Readback(
    ...
    tx.transition(TransactionStageV3::PostBootMeasuring)?;
    persist_or_fail(backend, tx)?;
    let observation = backend
        .collect_post_boot(manifest)
        .map_err(|primary| fail_transaction(backend, tx, primary, Vec::new()))?;
    validate_actual_post_boot_v3(manifest, tx, &observation)?;
    let bytes = serde_json::to_vec(&observation).map_err(|_| BootValidationV3Error::Payload)?;
    backend
        .persist_evidence("post-boot-evidence-v3.json", &bytes)
        .map_err(|_| BootValidationV3Error::Readback("persist post-boot evidence"))?;
    tx.payload.evidence_hashes.insert(
        "post-boot-evidence-v3.json".into(),
        canonical_bytes_sha256(&bytes),
    );
    tx.payload.post_boot_summary = Some(PostBootSummaryV3 {
    ...
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.transition(TransactionStageV3::PostBootValidated)?;
    persist_or_fail(backend, tx)?;
    Ok(observation)
}

pub fn select_baseline_rollback_v3<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &mut DurableTransactionV3,
    backend: &mut B,
) -> Result<(), BootValidationV3Error> {
    tx.transition(TransactionStageV3::BaselineSelecting)?;
    persist_or_fail(backend, tx)?;
    tx.record_intent(
    ...
    tx.transition(TransactionStageV3::BaselineSelected)?;
    persist_or_fail(backend, tx)
}
...
pub fn verify_then_cleanup_v3<B: BootLifecycleBackendV3>(
    manifest: &TieringBootValidationPreparedManifestV3,
    tx: &mut DurableTransactionV3,
    backend: &mut B,
) -> Result<(), BootValidationV3Error> {
    if tx.payload.stage == TransactionStageV3::BaselineSelected {
        tx.transition(TransactionStageV3::BaselineBoot)?;
        persist_or_fail(backend, tx)?;
    }
    let baseline = backend
        .collect_baseline(manifest)
        .map_err(|_| BootValidationV3Error::Readback("collect baseline"))?;
    verify_baseline_before_cleanup_v3(manifest, tx, &baseline)?;
    let bytes = serde_json::to_vec(&baseline).map_err(|_| BootValidationV3Error::Payload)?;
    backend
        .persist_evidence("baseline-restore-evidence-v3.json", &bytes)
        .map_err(|_| BootValidationV3Error::Readback("persist baseline evidence"))?;
    tx.payload.evidence_hashes.insert(
        "baseline-restore-evidence-v3.json".into(),
        canonical_bytes_sha256(&bytes),
    );
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.transition(TransactionStageV3::BaselineVerified)?;
    persist_or_fail(backend, tx)?;
    tx.transition(TransactionStageV3::Cleaning)?;
    persist_or_fail(backend, tx)?;
    ...
    let baseline_raw = tx
        ...
        .cloned()
        .ok_or(BootValidationV3Error::Readback("baseline raw evidence"))?;
    let profile_raw = tx
        .payload
        .evidence_hashes
        .get("post-boot-evidence-v3.json")
        .cloned()
        .ok_or(BootValidationV3Error::Readback("profile raw evidence"))?;
    let summary = tx
        .payload
        .post_boot_summary
        .clone()
        .ok_or(BootValidationV3Error::Readback("profile summary"))?;
    let common_archive = tx.payload.manifest_sha256.clone();
    let baseline_authority = SameHostZramBaselineEvidenceV2 {
        ...
        topology_sha256: canonical_json_sha256(&manifest.payload.topology),
        workload_sha256: canonical_json_sha256(&manifest.payload.workload),
        real: true,
        oom: false,
    ...
    let profile_authority = SameHostProfileEvidenceV3 {
        schema: PROFILE_BENCHMARK_EVIDENCE_V3.into(),
        validation_id: manifest.payload.validation_id.clone(),
    ...
    for (name, value) in [
        ...
        (
            "same-host-profile-v3.json",
            serde_json::to_vec(&profile_authority),
        ),
    ] {
        let bytes = value.map_err(|_| BootValidationV3Error::Payload)?;
        backend
            .persist_evidence(name, &bytes)
            .map_err(|_| BootValidationV3Error::Readback("persist same-host authority"))?;
        tx.payload
            .evidence_hashes
            .insert(name.into(), canonical_bytes_sha256(&bytes));
    }
    tx.payload_sha256 = canonical_json_sha256(&tx.payload);
    tx.transition(TransactionStageV3::Restored)?;
    persist_or_fail(backend, tx)?;
    backend
        .seal_archive(tx)
        .map_err(|_| BootValidationV3Error::Readback("seal evidence archive"))
}
...
mod tests {
    ...
    fn manifest() -> TieringBootValidationPreparedManifestV3 {
        ...
        let current = BootEntryIdentityV3 {
            id: "linux.conf".into(),
        ...
        let experimental = BootEntryIdentityV3 {
        ...
        let mut payload = PreparedManifestPayloadV3 {
            ...
            topology,
            baseline_swaps: vec![SwapIdentityV3 {
            ...
            protected_zram: zram,
            baseline_zswap: ZswapIdentityV3 {
        ...
        payload.owned_artifacts = vec![
            OwnedArtifactV3 {
                kind: OwnedArtifactKindV3::Type1Entry,
                path: entry_path,
            ...
            OwnedArtifactV3 {
                kind: OwnedArtifactKindV3::ValidationUnit,
                path: unit_path,
        ...
        TieringBootValidationPreparedManifestV3::seal(payload)
    }
    #[test]
    ...
    fn payload_tamper_is_rejected() {
        ...
        assert_eq!(m.validate(), Err(BootValidationV3Error::Payload));
    }
    #[test]
    fn unknown_zswap_parameter_is_rejected() {
        ...
        m = TieringBootValidationPreparedManifestV3::seal(m.payload);
        assert_eq!(m.validate(), Err(BootValidationV3Error::Zswap));
    }
    #[test]
    fn weak_topology_is_rejected() {
        ...
        m = TieringBootValidationPreparedManifestV3::seal(m.payload);
        assert_eq!(m.validate(), Err(BootValidationV3Error::Topology));
    }
    #[test]
    fn arbitrary_artifact_path_is_rejected() {
        ...
        m = TieringBootValidationPreparedManifestV3::seal(m.payload);
        assert_eq!(m.validate(), Err(BootValidationV3Error::Path));
    }
    #[test]
    fn type1_references_must_be_frozen() {
        ...
        m = TieringBootValidationPreparedManifestV3::seal(m.payload);
        assert_eq!(m.validate(), Err(BootValidationV3Error::Entry));
    }
    #[test]
    fn swap_must_outrank_protected_zram() {
        ...
        m = TieringBootValidationPreparedManifestV3::seal(m.payload);
        assert_eq!(m.validate(), Err(BootValidationV3Error::Swap));
    }
    fn observation(root: bool) -> PreflightObservationV3 {
        PreflightObservationV3 {
    ...
    fn preflights_are_non_mutating_and_sudo_bound() {
        let m = manifest();
        validate_preflight_v3(&m, &observation(false), false).unwrap();
        validate_preflight_v3(&m, &observation(true), true).unwrap();
        let mut o = observation(true);
        o.sudo_uid = Some(1);
        assert_eq!(
            validate_preflight_v3(&m, &o, true),
            Err(BootValidationV3Error::SudoIdentity)
        );
    }
    #[test]
    fn all_legal_transitions_and_illegal_transition() {
        let m = manifest();
        let mut t = DurableTransactionV3::new(&m, "boot-a".into()).unwrap();
        for s in [
            TransactionStageV3::RootPreflighted,
            TransactionStageV3::BaselineMeasuring,
            TransactionStageV3::BaselineMeasured,
            TransactionStageV3::Applying,
            TransactionStageV3::Applied,
            TransactionStageV3::OneShotSelecting,
            TransactionStageV3::OneShotSelected,
            TransactionStageV3::ExperimentalBootDetected,
            TransactionStageV3::ActivationPreparing,
            TransactionStageV3::ZswapDisabling,
            TransactionStageV3::ZswapParametersApplying,
            TransactionStageV3::ZswapEnabling,
            TransactionStageV3::SwapActivating,
            TransactionStageV3::ActivationVerified,
            TransactionStageV3::PostBootMeasuring,
            TransactionStageV3::PostBootValidated,
            TransactionStageV3::BaselineSelecting,
            TransactionStageV3::BaselineSelected,
            TransactionStageV3::BaselineBoot,
            TransactionStageV3::BaselineVerified,
            TransactionStageV3::Cleaning,
            TransactionStageV3::Restored,
        ] {
            t.transition(s).unwrap();
        }
        assert_eq!(
            t.transition(TransactionStageV3::Applying),
            Err(BootValidationV3Error::Transition)
        );
    }
    #[test]
    ...
    fn recovery_is_stage_aware_and_clean_is_noop() {
        assert_eq!(
            recovery_action_v3(TransactionStageV3::Applied, false),
            "remove_exact_owned_before_reboot"
        );
        assert_eq!(
            recovery_action_v3(TransactionStageV3::ActivationVerified, true),
            "select_exact_baseline_oneshot_preserve_artifacts"
        );
        assert_eq!(
            recovery_action_v3(TransactionStageV3::Restored, false),
            "no_op"
        );
    }
    fn post(
        m: &TieringBootValidationPreparedManifestV3,
    ) -> (DurableTransactionV3, ActualPostBootObservationV3) {
        let mut t = DurableTransactionV3::new(m, "a".into()).unwrap();
        for s in [
            TransactionStageV3::RootPreflighted,
            TransactionStageV3::BaselineMeasuring,
            TransactionStageV3::BaselineMeasured,
        ...
        (
            ...
            ActualPostBootObservationV3 {
                ...
                zswap: ZswapIdentityV3 {
                    parameters: p.experimental_zswap.clone(),
                },
                swaps: vec![SwapIdentityV3 {
                    active: true,
                    uuid: Some("swap".into()),
                    ..p.swapfile.clone()
                }],
                ...
                runtime_observation: RuntimeObserveOnlyEvidenceV3 {
    ...
    fn post_boot_accepts_only_actual_complete_measurement() {
        let m = manifest();
        let (t, o) = post(&m);
        validate_actual_post_boot_v3(&m, &t, &o).unwrap();
        let mut bad = o;
        bad.oom_kill = true;
        assert!(validate_actual_post_boot_v3(&m, &t, &bad).is_err());
    }
    #[test]
    fn post_boot_requires_boot_transition_and_marker() {
        ...
        let (t, mut o) = post(&m);
        o.boot_id = "a".into();
        assert!(validate_actual_post_boot_v3(&m, &t, &o).is_err());
        o.boot_id = "b".into();
    ...
    fn baseline_is_proven_before_cleanup() {
        ...
        let p = &m.payload;
        let o = BaselineRestoreObservationV3 {
        ...
        verify_baseline_before_cleanup_v3(&m, &t, &o).unwrap();
        let mut bad = o;
        bad.command_line.push_str(" marker");
        assert!(verify_baseline_before_cleanup_v3(&m, &t, &bad).is_err());
    }
    #[test]
    fn same_host_evidence_rejects_cross_host() {
        let b = SameHostZramBaselineEvidenceV2 {
            schema: ZRAM_BASELINE_EVIDENCE_V2.into(),
        ...
        let mut p = SameHostProfileEvidenceV3 {
            schema: PROFILE_BENCHMARK_EVIDENCE_V3.into(),
            validation_id: "v".into(),
            profile: StorageProfile::SataSsd,
        ...
        assert!(matching_same_host_evidence_v3(
            &b,
            &p,
    ...
    #[test]
    fn v3_unit_executes_only_the_root_owned_staged_helper() {
        let m = manifest();
        let unit = m
            .payload
            .owned_artifacts
            .iter()
            .find(|artifact| artifact.kind == OwnedArtifactKindV3::ValidationUnit)
            .unwrap();
    ...
    fn staged_helper_identity_is_not_self_declared_content() {
        ...
        let helper = m
            ...
            .owned_artifacts
            .iter_mut()
            .find(|artifact| artifact.kind == OwnedArtifactKindV3::HelperBinary)
            .unwrap();
        helper.content = vec![1, 2, 3];
        m = TieringBootValidationPreparedManifestV3::seal(m.payload);
        assert_eq!(
            m.validate(),
            Err(BootValidationV3Error::Identity("staged helper artifact"))
        );
    }
    ...
    fn full_preflight_readiness_is_auditable_conjunction() {
        ...
        assert_eq!(
            validate_preflight_v3(&m, &o, true),
            Err(BootValidationV3Error::Preflight(
                "serialized_readiness_contradiction".into()
            ))
        );
        o.ready = derived_preflight_ready_v3(&m, &o);
        assert!(!o.ready);
        assert!(validate_preflight_v3(&m, &o, true).is_err());
    }
    ...
    fn every_activation_substage_is_monotonic_and_duplicate_activation_is_illegal() {
        ...
        for pair in stages.windows(2) {
            assert!(legal_transition_v3(pair[0], pair[1]));
        }
        assert!(!legal_transition_v3(
            TransactionStageV3::ActivationVerified,
            TransactionStageV3::ActivationPreparing
        ));
    }
    ...
    fn post_boot_measuring_and_each_activation_failure_can_select_baseline() {
        for stage in [
            ...
            TransactionStageV3::ZswapEnabling,
            TransactionStageV3::SwapActivating,
            TransactionStageV3::ActivationFailed,
            TransactionStageV3::PostBootMeasuring,
        ] {
            assert!(legal_transition_v3(
                stage,
    ...
    fn mutation_intent_is_bound_to_the_current_boot_id() {
        let m = manifest();
        let mut tx = DurableTransactionV3::new(&m, "boot-a".into()).unwrap();
    ...
    fn apply_cannot_bypass_the_sealed_same_host_baseline() {
        ...
        assert_eq!(
            apply_exact_transaction_v3(&m, &mut tx, &mut backend),
            Err(BootValidationV3Error::Transition)
        );
    ...
    fn noisy_physical_write_evidence_never_authorizes_recommendation() {
        let (b, mut p) = {
            ...
            let profile = SameHostProfileEvidenceV3 {
                schema: PROFILE_BENCHMARK_EVIDENCE_V3.into(),
    ...
    fn config_only_observe_claim_cannot_authorize_post_boot() {
        ...
        assert!(validate_actual_post_boot_v3(&m, &tx, &actual).is_err());
    }
    ...
    #[test]
    fn v1_and_v2_contracts_are_historical_not_v3_authority() {
        let m = manifest();
        let json = serde_json::to_value(&m).unwrap();
        assert_eq!(
            json.get("schema").and_then(serde_json::Value::as_str),
            Some(PREPARED_MANIFEST_SCHEMA_V3)
        );
        assert_ne!(
            PREPARED_MANIFEST_SCHEMA_V3,
            crate::boot_validation_v2::PREPARED_MANIFEST_SCHEMA_V2
        );
        assert_ne!(
            BOOT_VALIDATION_CONTRACT_VERSION_V3,
    ...
    struct FakeLifecycle {
        persisted: Vec<TransactionStageV3>,
        ...
        post: Option<ActualPostBootObservationV3>,
        baseline: Option<BaselineRestoreObservationV3>,
        mutations: usize,
    }

    impl BootLifecycleBackendV3 for FakeLifecycle {
        fn persist_transaction(&mut self, t: &DurableTransactionV3) -> Result<(), String> {
        ...
        fn create_transaction_root(
            &mut self,
            _: &TieringBootValidationPreparedManifestV3,
        ) -> Result<(), String> {
            self.mutations += 1;
            Ok(())
        }
        ...
        fn create_artifact(&mut self, a: &OwnedArtifactV3) -> Result<(), String> {
        ...
        fn remove_artifact(&mut self, a: &OwnedArtifactV3) -> Result<(), String> {
            self.mutations += 1;
        ...
        fn swapfile_absent(&self, _: &TieringBootValidationPreparedManifestV3) -> bool {
            !self.swap
        }
        fn finalize_runtime_cleanup(
            &mut self,
        ...
        fn seal_archive(&mut self, _: &DurableTransactionV3) -> Result<(), String> {
            Ok(())
        }
    }
    ...
    fn initialized_tx(
        ...
        initialize_and_measure_baseline_v3(m, &observation(true), "boot-a".into(), b).unwrap()
    }
    ...
    fn apply_persists_before_each_bounded_mutation() {
        ...
        apply_exact_transaction_v3(&m, &mut tx, &mut b).unwrap();
        assert_eq!(tx.payload.stage, TransactionStageV3::Applied);
        assert!(b.swap);
        assert_eq!(b.artifacts.len(), 3);
        for required in [
            TransactionStageV3::Prepared,
            TransactionStageV3::RootPreflighted,
            TransactionStageV3::BaselineMeasuring,
            TransactionStageV3::BaselineMeasured,
            TransactionStageV3::Applying,
    ...
    fn partial_apply_preserves_primary_error_and_rolls_back_exact_owned() {
        ...
        let mut tx = initialized_tx(&m, &mut b);
        assert!(apply_exact_transaction_v3(&m, &mut tx, &mut b).is_err());
        assert!(!b.swap);
        assert!(b.artifacts.is_empty());
        assert_eq!(b.persisted.last(), Some(&TransactionStageV3::Failed));
    }
    ...
    fn oneshot_exit_without_exact_readback_is_rejected() {
        ...
        assert!(select_exact_one_shot_v3(&m, &mut tx, &mut b).is_err());
        assert_eq!(
    ...
    fn post_boot_api_collects_from_backend_not_caller() {
        ...
        tx.payload.stage = TransactionStageV3::ActivationVerified;
        tx.payload_sha256 = canonical_json_sha256(&tx.payload);
        let mut b = FakeLifecycle {
            post: Some(actual),
            ..Default::default()
        ...
        let observed = collect_and_validate_post_boot_v3(&m, &mut tx, &mut b).unwrap();
        assert!(observed.workload_completed);
        assert_eq!(tx.payload.stage, TransactionStageV3::PostBootValidated);
    }
    ...
    fn failed_baseline_verification_deletes_nothing() {
        ...
        b.swap = true;
        let mut tx = DurableTransactionV3::new(&m, "a".into()).unwrap();
        for s in [
            TransactionStageV3::RootPreflighted,
            TransactionStageV3::BaselineMeasuring,
            TransactionStageV3::BaselineMeasured,
        ...
        assert!(verify_then_cleanup_v3(&m, &mut tx, &mut b).is_err());
        assert_eq!(b.artifacts.len(), 3);
        assert!(b.swap);
    }
}
